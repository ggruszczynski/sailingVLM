...
from InputData.straight_wing_vlm_case import *
...
CX_vlm = sail_mirror_multiplier*inviscid_flow_results.F_xyz_total[0] / (q*S)
CY_vlm = sail_mirror_multiplier*inviscid_flow_results.F_xyz_total[1] / (q*S)
CZ_vlm = sail_mirror_multiplier*inviscid_flow_results.F_xyz_total[2] / (q*S)

a_VLM = CY_vlm / np.deg2rad(AoA_deg)
CL_expected, CD_ind_expected, a_expected = get_CL_CD_free_wing(AR, AoA_deg)

print(f"\nAspect Ratio {AR}")
print(f"CL_vlm      {CY_vlm:.6f}        \t CL_expected      {CL_expected:.6f}")
print(f"CD_vlm      {CX_vlm:.6f}        \t CD_ind_expected  {CD_ind_expected:.6f}")
print(f"a_VLM       {a_VLM:.6f}         \t a_expected       {a_expected:.6f}")
